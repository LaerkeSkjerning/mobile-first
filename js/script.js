...
if (window.innerWidth < 1025){ /* If-statement, der frasorterer de skærme, der er større end 1024px. Ved en skærm, der er større end 1024px kører karrusellen ikke */
    document.addEventListener("DOMContentLoaded", function() { /* listener, der venter paa at al HTML-indholdet er loaded foer funktionen starter */
        const inner = document.querySelector(".indre-karrusel"); /* querySelector henter det foerste element, der stemmer overens med argumentet - her er det klassen 'indre-karrusel' */
        const pages = document.querySelectorAll(".karrusel-side"); /* querySelectorAll henter alle elementer med klassen 'karrusel-side' */
        const colors = ["var(--primaer-lyseroed)", "var(--primaer-gul)", "var(--primaer-lyseroed)"]; /* Array indeholdende baggrundsfarverne. Der har hhv. indexvaerdierne 0, 1 og 2 */
        let currentIndex = 0; /* currentIndex skal aendre værdi og variablen er derfor en let. Variablen bruges til at skifte side i karrusellen */
        let direction = 1; /* direction skal aendre vaerdi og variablen er derfor en let. Variablen bruges til at skifte side i karrusellen */
        ...
        function translatePages() {
            let offset = -currentIndex * 100; /* offset faar en negativ vaerdi for at flytte indholdet til venstre. Vi ganger med 100 for at give indholdet en bestemt stoerrelse og paa den maade bestemme, hvor meget siden skal flytte sig for at skifte helt. Da hver side skal fylde 100% forskyder ved ogsaa med 100%*/
